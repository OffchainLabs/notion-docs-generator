...
export async function queryDatabase(
  ...
  try {
    ...
    if (err.status == 502 || err.status == 500) {
      ...
      if (attemptsLeft > 0) {
        console.log(
          `DQ query ${err.status} error: ${attemptsLeft} attempts remaining.`
        );
        console.log(
          ` Waiting ${options.delaySeconds} seconds and trying again`
        );
        await wait(options.delaySeconds * 1000);
        return queryDatabase(client, params, {
          ...options,
...
export async function getPageWithBlocks(
  ...
  try {
    ...
    if (err.status == 502 || err.status == 500) {
      ...
      if (attemptsLeft > 0) {
        console.log(
          `Get pages ${err.status} error: ${attemptsLeft} attempts remaining.`
        );
        console.log(
          ` Waiting ${options.delaySeconds} seconds and trying again`
        );
        await wait(options.delaySeconds * 1000);
        return getPageWithBlocks(client, pageId, {
          ...options,
          attempts: attemptsLeft,
        });
      }
...
async function getBlockChildren(
  ...
  try {
    ...
    if (err.status == 502 || err.status == 500) {
      ...
      if (attemptsLeft > 0) {
        console.log(
          `Get Block Children ${err.status} error: ${attemptsLeft} attempts remaining.`
        );
        console.log(
          ` Waiting ${options.delaySeconds} seconds and trying again`
        );
        await wait(options.delaySeconds * 1000);
        return getBlockChildren(client, block_id, {
          ...options,
          attempts: attemptsLeft,
        });
      }
